...
Lemory는 보스 노트 자체를 1위로, ~0.1초에, LLM 0회로 돌려줍니다.
...
`lemory ask "..."` 한 줄은 서버 없이도 됩니다. (언제 켜두고 언제 다시 색인하는지
전체 흐름은 [가이드](docs/GUIDE.ko.md#4-쓰는-법--한-번-설정-그다음엔-계속) 참고.)

인제스트에 LLM이 돌지 않아서 **노트 1,000개 색인 = LLM 호출 0회**입니다:
...
비서는 이제 대화 안에서 **읽고 씁니다**: "…라고 기억해줘"라고 말하면 그 자리에서 볼트 노트로 저장되고(승인제가 켜져 있으면 대기열로), "그건 언제였지?" 같은 후속 질문은 앞 질문과 묶어 검색합니다. 첫 턴엔 볼트 상황 요약이 자동 주입되고, `/검색` `/기억` `/최근` 명령도 됩니다.
...
일반 에이전트 축도 측정돼 있습니다: **AgentMemQA**([§7f](BENCHMARKS.md)) - 12주치 업무 비서 세션(영한 혼용 기술 대화, 코드블록, 에러 트레이스)에 번복된-결정 함정을 채점으로 심은 벤치. 키리스 하이브리드 doc@1 **0.956**, 자기 레그 모두 상회. 역발견도 정직하게: cross-encoder 리랭커는 시간맹이라 갱신된 사실에서 recency를 되돌립니다(decision 0.80→0.50) - 리랭커가 기본 off인 이유가 하나 더 늘었습니다.

실채팅의 지저분함(번복·농담 페이크·어휘 오염 잡담)도 가정이 아니라
측정입니다: RoleMemQA-messy에서 번복 소환 1.000(옛 값 함정 0), 전체 doc@1
0.820 vs 클린 0.984 ([§7e](BENCHMARKS.md)).
...
**0.983**(any) / 0.904(strict). 한국어 특화 e5 기본이 옛 MiniLM(0.972/0.857)을
영어에서도 오히려 앞섭니다. 헤드라인들이 쓰는 지표(any)에서 풀셋, 제로 API로
그 수준이고, 더 엄격한 숫자를 앞세워 둘 다 공개합니다.
...
Lemory가 "맞는 세션이 소환되는가"에 **doc@1 0.984** (업데이트형 1.000,
옛-선호 함정률 **0**), 자기 vector(0.938)·BM25(0.820) 레그를 모두 이기며
~1ms. 이 벤치가 잡아낸 결함 2건(recency는 벽시계가 아니라 기억 타임라인
기준이어야 / 채팅 추임새가 렉시컬 레그를 점령하면 안 됨)도 바로
수정했습니다.
...
각 클립은 실제 CLI 출력을 **글자 그대로**(verbatim) 다시 타이핑한 것입니다.
목업 없음 — 재생성: `docs/assets/make_gifs.py` (캡처 원문이 스크립트에 포함).
