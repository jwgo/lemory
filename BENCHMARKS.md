...
실패하는 축(허브 문서가 33k 청크의 상위권을 채운다); (b) 마스킹 문항에서
하이브리드(0.461)가 자기 BM25 레그(0.735)보다 낮다 — 인용 식별자가 희귀
토큰인데 커버리지 게이트가 전체 토큰 기준이라 pin이 안 걸리고 약한 벡터
레그가 융합을 희석한다. IDF 인지 커버리지가 다음 라운드 후보. 무응답
8문항은 e2e 채점용 플래그(answerable=false)로 분리.
