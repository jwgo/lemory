...
**Smart Connections (2.4.x)** is measured as its retrieval core: the default
...
|---|---|---|---|---|---|---|
| **Lemory** (hybrid) | **0.977** | **1.000** | **1.000 (0.000)** | 0.938 | **1.000** | **1.000** |
...
| (keyless hybrid) | doc@1 | doc@8 | 함정↑골드 | 비고 |
|---|---|---|---|---|
| retraction (번복) | **1.000** | 1.000 | 0.000 | 정보량 prior 전 0.875/0.125; 아래 원장 |
| joke / update | **1.000** | 1.000 | 0.000 | |
| episodic | **0.500** | **0.875** | - | 정보량 prior 전 0.458/0.792 (클린 0.938) |
| long | **1.000** | 1.000 | - | 정보량 prior 전 0.958 |
| twohop | 0.375 | 1.000 | - | fs 0.875 |
| **전체** | **0.867** | **0.977** | | 정보량 prior 전 0.836/0.961, 클린 0.977 |

doc@8(정답이 LLM 컨텍스트에 도달하는가)이 실전에서 중요한 지표다. episodic
doc@8이 0.792 → **0.875**로 올랐고, 이 이득의 정체를 다음 절에서 실측으로
분해한다.

### 정보량 prior: boilerplate 질문의 벡터 near-tie 깨기 (신규, 실측)

에피소드 회상 질문("우리 약속이 뭐였지?", "지어준 별명은?")은 **어휘적
변별력이 0**이다. 내용 토큰(약속/별명)이 전부 코퍼스 boilerplate라 BM25가
쓸모없고, 벡터 레그만 남는다. 그런데 벡터마저 함정에 걸린다: 필러
"기억해 둘게, 약속!"이 골드 팩트 "우리 약속 하나 하자. 장마 끝나면
자전거 여행 가기"보다 질문 단어를 더 그대로 되풀이해서 코사인이 더 높다
(실측: 골드 버스트 벡터 rank 1, 필러 버스트 rank 0).

해법: 그런 질문에서만 벡터 레그를 **희소 내용 정보량**으로 재정렬한다
(`informativeness_prior`, 기본 0.6). 각 후보 청크의 "코퍼스에서 희소한
내용어 비율"(fts5vocab의 청크 df로 계산, 질의당 캐시 1회 · 지연 +0.3ms)을
코사인에 곱한다. 골드 팩트는 희소어(장마·자전거·여행)를 담아 점수가 오르고,
필러("기억해 둘게")는 희소어가 0이라 그대로다. 버스트가 canonicalize되기
**전** raw 벡터 레그에 적용해, 오른 골드 버스트가 자기 패킹 형제를 끌어올린다.

두 겹 게이트로 정상 질문은 절대 건드리지 않는다: 엄격 게이트(모든 토큰이
색인됐고 common)는 BM25 damp를, 느슨한 게이트(색인된 변별 토큰이 없음 ·
패러프레이즈로 노트에 없는 토큰은 허용)는 정보량 prior만 발동한다.

| 벤치 | prior 0.0 | prior 0.6 | Δ |
|---|---|---|---|
| RoleMemQA-messy episodic doc@8 | 0.792 | **0.875** | +8.3pt |
| RoleMemQA-messy episodic doc@1 | 0.458 | **0.500** | +4.2pt |
| RoleMemQA-messy retraction 함정↑골드 | 0.125 | **0.000** | 함정 제거 |
| RoleMemQA-messy 전체 doc@8 | 0.961 | **0.977** | +1.6pt |
| RoleMemQA-clean 전체 doc@1 | 0.9766 | 0.9766 | **0 (동일)** |
| KorQuAD dev 400 recall@1 | 0.930 | 0.930 | **0 (동일)** |
| AgentMemQA 전체 doc@1 | 0.9778 | 0.9778 | **0 (동일)** |

게이트가 참조형/엔티티 질문(KorQuAD, clean, AgentMemQA)엔 한 번도 안 걸려
그 벤치들은 수치가 비트 단위로 동일하다. 이득은 순수하게 boilerplate
에피소드 질문에서만 나온다. p50 5.3 → 5.6ms.

### 대화 버스트 청킹 (Cerebras KB에서 이식, 실측 원장 전부 공개)

Cerebras가 사내 지식베이스에서 쓰는 burst 임베딩(같은 화자의 연속 발화 =
임베딩 단위)을 Lemory식으로 이식했다: 채팅 레이아웃 노트(`**나**: …`
문단이 과반)는 기존 패킹 청크에 **더해** 신호가 있는 화자 버스트(내용
≥25자 또는 숫자 포함; 잡담 게이트)를 집중 청크로 추가 색인한다
(`chat_burst_chunking`, 기본 on). 검색 융합에서 버스트는 **자기 패킹
형제 청크의 랭크를 올리는 별칭**으로만 작동한다. 티켓을 늘리지 않는다.

이 설계는 4번의 실측 실패를 거쳐 수렴했고, 전부 기록한다:
1. 버스트가 패킹을 **대체** → update 함정 역전 100% (문서 단위 키워드
   집적 상실). 2. 버스트+패킹 동시 융합 → 같은 노트가 RRF 티켓 2장을
   쥐고 0.0004 차 융합 마진을 뒤집음 (클린 long 회귀). 3. 레그별 중복
   **삭제** → 레그마다 다른 생존자를 골라 한 노트의 융합 질량이 두 청크로
   쪼개짐 (전 벤치 doc@8 붕괴). 4. 벡터 전용(FTS 제외) → messy 이득의
   원천이 어휘 레그였음이 드러남 (0.813, flat보다 나쁨).

최종 원장 (전부 같은 keyless local 하니스, flat = 청킹 off 대조군):

| 벤치 | flat | 버스트(별칭) | Δ |
|---|---|---|---|
| RoleMemQA-messy 전체 doc@1 | 0.820 | **0.836** | +1.6pt (long +6.3, twohop fs 0.75→0.875) |
| AgentMemQA 전체 doc@1 | 0.956 | **0.978** | +2.2pt, decision 함정 0.2→**0.0** |
| RoleMemQA-clean 전체 doc@1 | 0.984 | 0.977 | **-0.8pt** (long 1문항) |
| messy retraction 함정↑골드 | 0.000 | 0.125 | **1문항 회귀**: 융합 점수차 0.0001의 동전던지기 |
| KorQuAD dev 400 r@1 | 0.935 | 0.930 | 동률 타이브레이크 노이즈; 4,000문항에서 **0.9795 vs 0.9778로 역전** (버스트 청크 0개, 아래) |

LongMemEval/LOCOMO는 세션이 문단 하나로 합쳐지는 레이아웃이라 버스트
청킹의 영향권 밖이고, 유일하게 바뀐 경로(recency 표지 질문의 핀 깊은
스캔)에 해당하는 LME 36문항을 표적 재실행해 커밋된 470문항 원장과 문항
단위로 대조했다: **지표 차이 0건** (`scratchpad 재현: 표지 질문만 재색인
후 all/any@5/10 비교`). KorQuAD엔 버스트 청크가 0개인데도 수치가 움직인 이유: 핀 후보 순회가
사전순(사실상 SQL 임의 순서)에서 BM25 랭크순으로 바뀌며 커버리지 동률
45/400건의 선택이 갈렸다. 400문항 표본에선 -2문항, 10배 표본(4,000)에선
+7문항; 임의 순서 대신 랭크순을 채택하고 원장에 남긴다. 번복 함정
1문항 회귀는 버스트가 짧고 선명한 옛 진술("이름은 미네르바야")을
정정문("뻥이었어, 진짜는 임가을")보다 잘 부스트하는 구조적 편향이다.
"진짜/실제"를 최신성 신호로 넣는 패치는 joke 함정을 8/8→3/8로 붕괴시켜
(농담 세션이 대개 더 최신) 폐기했다. 이득은 챙기고 회귀는 공개한다.
...
**버스트 청킹 채택 후 재검토 (Cerebras 대비 최종 판단).** Cerebras는
원문 대신 LLM-정규화 데이터를 임베딩한다. 우리 축에서 그 대응물은
distill(LLM 정규화, 기본 off 유지)과 버스트 청킹(LLM 0회로 임베딩
단위를 사실 단위에 근접시킴, 기본 on)이다. 버스트 청킹이 messy +1.6pt /
AgentMemQA +2.2pt를 무LLM으로 회수한 지금, distill을 기본 경로로 올릴
근거는 더 약해졌다: 다이제스트가 골드 세션을 대체하는 실패 모드
(doc@1 0.820→0.664)는 구조적이고, 증류가 주던 집중-텍스트 이득의
상당 부분을 버스트가 인제스트 시점에 공짜로 제공한다. 결론: 정규화
임베딩 갭은 **버스트 청킹으로 닫고**, distill은 사람이 읽는 팩트시트
용도의 옵트인으로 남긴다.
...
|---|---|---|---|---|---|
| **Lemory** (hybrid) | **0.978** | **1.000** | **0.90 (0.00)** | **1.000** | 6.4ms |
...
인용하는 후보들 사이의 서수 결정이라 결정적이어야 맞다. 이후 대화 버스트
청킹(§7e)이 decision 함정을 0.20→**0.00**으로 마저 내렸다. 세션 노트가
채팅 레이아웃이라 이 벤치도 버스트 청킹의 수혜 축이다 (전체 doc@1
0.956→0.978).

**정직한 경계, 그리고 리랭커의 시간맹.** 남은 decision 실패 2건은 정정이
극단적 패러프레이즈라("뒤집는다, Y로 최종 확정" vs 질문 "뭐 쓰기로
했지?") 커버리지가 핀 게이트 미달인 케이스다. 답은 rank-2에 존재한다
...
**시간맹 교정 재시도, 그리고 반증.** 리랭커 점수에 융합 단계와 동일한
감쇠(0.6×) recency 배수를 곱하는 "시간 보존 블렌드"를 구현해 실제
Qwen3-Reranker-0.6B로 같은 볼트에서 3팔 A/B를 돌렸다: 리랭커 off
0.978(decision 9/10) / 리랭커 on 0.889(5/10) / 리랭커 on + 블렌드
**0.867(4/10)**. 블렌드가 오히려 더 나빴다. 원인은 이중: decision
질문 다수가 최신성 표지 없이 최신 결정을 묻고(블렌드 미발동), 표지가
있는 질문에선 배수가 신선-오답 세션을 되올렸다. 블렌드 코드는 되돌렸고
(commit revert), 결론은 강화됐다: **cross-encoder는 사후 보정으로
시간축을 복구할 수 없으며 기본 off가 맞다.** 13.5s/질문의 지연은 덤이다.
