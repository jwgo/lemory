...
|---|---|---|---|---|---|---|---|---|---|
| **Lemory** (hybrid+graph) | **0.526** | 0.738 | **0.747** | 0.461 | 0.797 (**0.141**) | 0.807 | **0.964** | 0.827 | **0.591** |
...
<sub>† 층화 시드 샘플(지연 제약). **동일 샘플**에서 Lemory: n=84 doc8
0.738/17.7ms, n=280 doc8 0.764/16.4ms. 즉 qmd의 헤드라인 모드는 84문항
샘플에서 +3.6pt(문항 3개, 소표본 노이즈 범위)를 얻는 대가로 **~3,400×**의
지연을 낸다 — GPU라면 ~1-2s/query로 줄지만 여전히 ~100×. 벡터 모드는
품질(-10.7pt)과 속도(260×) 모두 밀린다.</sub>

정직한 미해결 과제 두 개: (a) 2-hop full-support 0.141 — 모든 시스템이
실패하는 축(허브 문서가 33k 청크의 상위권을 채운다; qmd query도 fs 0.333
에 60초를 쓴다); (b) 마스킹 문항에서 하이브리드(0.461)가 자기 BM25
레그(0.735)보다 낮다 — 인용 식별자가 희귀 토큰인데 커버리지 게이트가 전체
토큰 기준이라 pin이 안 걸리고 약한 벡터 레그가 융합을 희석한다. IDF 인지
커버리지가 다음 라운드 후보. 무응답 8문항은 e2e 채점용 플래그
(answerable=false)로 분리.
