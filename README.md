...
**기억 · 지식 · 스킬을 내 폴더의 마크다운으로 통합 — 호스팅 DB도, 계정도, 쿼터도 없이, 전부 실측으로 증명하면서.**
...
**Lemory는 AI 에이전트를 위한 로컬 컨텍스트 데이터베이스예요.** 에이전트가
필요로 하는 컨텍스트 — 기억, 지식, 스킬 — 를 내 폴더의 마크다운 파일 하나의
체계로 통합해요. Claude Code, Claude Desktop, Cursor, 직접 만든 에이전트가
매 세션 낯선 사람으로 시작하지 않게 하고, 마크다운 노트가 이미
있다면(옵시디언이든 그냥 폴더든) 그게 곧바로 컨텍스트가 돼요.

2026년, 시장은 이 방향으로 수렴했어요. ByteDance의 OpenViking이 "컨텍스트
데이터베이스"를 만들며 벡터 스토어 대신 **파일시스템 패러다임**을 택했고,
Tencent의 Agent Memory가 기억 피라미드를, Vectorize의 Hindsight가
retain-recall-reflect를 택했죠. 방향은 전부 맞아요. 그런데 그들의
파일시스템은 `viking://` 같은 **가상 경로** — 밑에는 여전히 그들의 DB가
있어요. Lemory는 반대쪽 끝에서 출발했어요: **당신이 이미 가진 진짜 파일이
데이터베이스예요.** `ls`, `grep`, `git`, 옵시디언이 그대로 관리 도구이고,
떠나는 비용은 0이에요. 그리고 그 선택이 성능을 깎지 않는다는 걸 — 오히려
이긴다는 걸 — 돌릴 수 있는 경쟁자 전부와 같은 하네스에서 측정해서 보여줘요.

- **하나의 파일 체계에 모든 컨텍스트.** 타입 있는 기억 파편, 장면 서사,
  페르소나, 추출된 스킬, 그리고 당신의 노트 — 전부 볼트 안의 평범한
  `.md`예요. 가상 계층이 없으니 에이전트용 뷰와 사람용 뷰가 같은 파일이고,
  `rm` 한 번이면 지워져요. 계정도, 파편 쿼터도, 이사 비용도 없어요.
- **계층 로딩(L0/L1/L2)으로 토큰을 아껴요.** 모든 노트를 L0 한 줄 요약,
  L1 개요(제목 골격 + 섹션 도입부), L2 원문 세 단계로 읽을 수 있어요.
  다른 컨텍스트 DB는 이 계층을 **쓰기 시점에 LLM으로 생성**하지만, Lemory는
  **읽기 시점에 결정적으로 유도**해요 — LLM 0회, 원본과 어긋날 수 없음.
  세션 부트는 피라미드(페르소나+장면 지도)가 맡아서 원문 덤프의 1/48.8
  토큰으로 시작해요.
- **검색이 실측으로 고성능이에요.** 의미 + 한국어 특화 키워드 + `[[위키링크]]`
  그래프 + 시간 창, 4개 다리의 하이브리드 융합. 같은 하네스에서 키리스
  0.983@40ms, 임베딩 없는 fast 모드 0.967@6.9ms — 지는 항목까지 전부
  공개해요.
- **관찰 가능해요.** 모든 질의, AI가 쓴 모든 노트(클릭 한 번 되돌리기),
  클라이언트별 사용량이 타임라인에 남아요. 전부 내 컴퓨터의 SQLite 파일
  하나예요.
- **스스로 진화해요.** 세션이 끝나면 대화가 사실 아톰으로 증류되고, 장면
  서사와 페르소나로 승격돼요(`auto_consolidate` 한 줄이면 자동). LLM이
  없으면 결정적 폴백으로 — 완전 오프라인에서도 피라미드가 살아 있어요.
...
툴은 20개예요. 읽기: `search_notes` · `ask_notes` · `recent_notes` ·
...
`read_note`로, 원본은 검색으로 필요할 때만 파요. 서버를 켜두면
`auto_consolidate = true` 한 줄로 승격이 자동이 돼요 · 새 기억이 몇 분
조용해지면 알아서 장면·페르소나로 올라가요(설정 탭 토글).

실측했어요(RoleMemQA 8관계×30세션, 재현: `benchmarks/run_pyramid.py`):
항상 주입되는 부트 컨텍스트가 **1,345토큰 · 원문 덤프(65,602토큰)의
1/48.8**인데 페르소나 사실 질문의 34.7%가 그 안에서 바로 답혀요. 장면
하나만 드릴하면 66.7% @ 2,084토큰, 나머지는 검색층이 100%로 받아요. 그쪽
발표치(PersonaMem +59%)는 리포에 하네스가 없어 재현이 불가능해요 · 우리
수치는 전부 스크립트 하나로 다시 나와요.
...
`proxy_upstream`으로 바꿔요 · 기본 OpenAI지만 OpenAI 호환이면 뭐든 돼요.
Ollama(`http://localhost:11434/v1`)를 가리키면 **모델까지 포함해 완전
로컬인 메모리 프록시**가 돼요.
...
*미들웨어에 뭐가 오갔는지* 보고 바로 손보는 작업 공간이에요. 데스크톱
지식앱의 문법 — 토큰 기반 디자인 시스템(라이트/다크), ⌘K 팔레트,
⌘1–8 이동, `?` 단축키 도움말, 하단 상태바 — 을 웹 콘솔 비용으로 담았어요.

- **현황**: 타임라인이에요. AI 메모리 피드(되돌리기 포함), 최근 질의와
  출처, 클라이언트별 사용량, 색인 활동을 볼 수 있어요.
- **지식**: 3열 워크스페이스예요(열 폭은 드래그로 조절하고 기억돼요).
  목록엔 노트마다 한 줄 미리보기, 상세엔 스티키 헤더 · 클릭되는
  경로 브레드크럼 · 긴 노트 목차(읽는 위치 따라오는 스크롤 스파이).
  그리고 **직접 써요**: 새 노트 · 이름 변경 · 휴지통, 구문 하이라이트
  에디터 + 라이브 미리보기, `[[위키링크]]` 자동완성, 1.5초 자동 저장
  (디스크 충돌은 409로 보호), frontmatter **속성 인스펙터**(태그는 칩으로
  더하고 빼요).
- **기억**: 피라미드(페르소나 · 장면 · 앵커 · 케이스 · 스킬)와 타입별
  회상이에요.
- **그래프**: 볼트 전체 상호작용 그래프예요.
- **건강**: 승인 대기 기억, 모순 쌍, 드리프트, 링크 제안을 한 화면에서
  처리해요.
- **검색**: 하이브리드/벡터/키워드를 바꿔가며 점수와 속도를 볼 수 있는
  실험실이에요. 빈 화면 대신 내 볼트로 만든 튜토리얼이 떠요.
- **비서**: 온디바이스 모델로 볼트에 대해 스트리밍 대화해요(출처 인용).
- **설정**: 검색 옵션과 모델 선택을 바로 적용해요. 타임라인 기록 여부도
  설정이고(`event_log`), 전부 내 SQLite 파일에만 남아요.
