...
번복·농담·잡담 오염을 심었고, doc@1 0.836 vs 깨끗한 노트 0.977. 이 격차를
줄인 버스트 청킹의 개선/후퇴 내역 전체가 [§7e](BENCHMARKS.md)에 있어요.
...
- **`lemory search --fast`** — 즉답 경로예요. 임베딩 없이 키워드 검색 +
  ...
  (하이브리드는 21ms에 0.967 — 말 바꿔 묻기, 다른 언어, 멀티홉엔 벡터가
  필요해서 기본은 하이브리드예요). 타이핑 즉시 검색창용이에요.
...
`uv venv && uv pip install -e ".[dev,mcp,local,pdf]" && pytest` — 테스트
400개, 완전 오프라인이에요. [CONTRIBUTING.md](CONTRIBUTING.md) · 한국어
이슈/PR 환영해요.
